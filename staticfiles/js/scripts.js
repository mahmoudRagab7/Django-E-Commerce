    ...
    try {
        ...
        data.products.forEach(product => {
            ...
            div.innerHTML = `
                ...
                <button onclick="addToCart(${product._id}, '${product.name}', ${product.price}, '${product.image}')" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 mt-2">Add to Cart</button>
            `;
        ...
        document.getElementById('next-page').onclick = () => loadProducts(data.page - 0 + 1);
    } catch (error) {
...
async function loadProductDetail() {
    ...
    try {
        ...
        detail.innerHTML = `
            ...
            <ul>${product.reviews.map(r => `<li>${r.name}: ${r.rating}/5 - ${r.comment}</li>`).join('')}</ul>
        `;
    } catch (error) {
...
console.log('Scripts.js functions defined:', {
    checkAuth: typeof checkAuth,
    registerUser: typeof registerUser,
    loginUser: typeof loginUser,
    loadProducts: typeof loadProducts,
    addToCart: typeof addToCart,
    loadProductDetail: typeof loadProductDetail,
